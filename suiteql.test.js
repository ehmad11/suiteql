...
describe("Netsuite SuiteQL Tests", () => {
  ...
  test("it should throw error if query is not string", async () => {
    expect.assertions(1);
    await expect(suiteQL.query(123)).rejects.toThrow(Error);
  });
  ...
  test("it should get all 30 records from transaction table using queryAll", (done) => {
    expect.assertions(1);
    let items = [];
